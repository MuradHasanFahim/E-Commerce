    ...
    <form onSubmit={onSubmitHandler}  className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
      ...
      <div className='flex flex-col gap-4 w-full sm:max-w-120'>
        <div className='text-xl sm:text-2xl my-3'>
          <Title text1={'DELIVARY'} text2={'INFO'}/>
        ...
        <input onChange={onChangeHandler} name='email' value={formData.email} className='border border-gray-300 rounded py-1.5 px-3.5 w-full' type="email" placeholder='Your Email Address' />
      ...
      <div className='mt-8'>
        ...
        <div className='mt-12'>
          ...
          <div className='flex gap-3 flex-col lg:flex-row'>
            ...
            <div onClick={()=>setMethod('stripe')} className='flex items-center gap-3 shadow-md p-2 px-3 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method==='stripe'?'bg-green-500':''}`}></p>
              <img className='h-5 mx-4' src={assets.stripe_logo} alt="" />

            </div>

              <div onClick={()=>setMethod('razorpay')} className='flex items-center gap-3 shadow-md p-2 px-3 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method==='razorpay'?'bg-green-500':''}`}></p>
              <img className='h-5 mx-4' src={assets.razorpay_logo} alt="" />

            </div>

              <div onClick={()=>setMethod('cod')} className='flex items-center gap-3 shadow-md p-2 px-3 cursor-pointer'>
          ...
          <div className='w-full text-end mt-8'>
            <button className='border bg-black text-white px-16 py-3 text-sm cursor-pointer'>PLACE ORDER</button>
